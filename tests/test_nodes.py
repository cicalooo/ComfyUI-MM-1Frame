...
def test_one_frame_node_schemas():
    latent_schema = mm_nodes.EmptyMiniMaxH3OneFrameLatent.define_schema()
    assert latent_schema.node_id == "MM1FrameEmptyMiniMaxH3LatentAV"
    assert [input_.id for input_ in latent_schema.inputs] == ["width", "height", "batch_size"]
    assert [output.io_type for output in latent_schema.outputs] == ["LATENT"]

    ref_schema = mm_nodes.MiniMaxH3ReferenceToImageOneFrame.define_schema()
    assert ref_schema.node_id == "MM1FrameMiniMaxH3ReferenceToImage"
    assert [input_.id for input_ in ref_schema.inputs[:6]] == [
        "clip",
        "vae",
        "prompt",
        "width",
    ...
    assert [output.io_type for output in ref_schema.outputs] == ["CONDITIONING", "LATENT"]


def test_extension_registers_only_original_one_frame_nodes():
